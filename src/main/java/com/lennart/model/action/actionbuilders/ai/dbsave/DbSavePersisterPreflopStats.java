...
public class DbSavePersisterPreflopStats {
    ...
    public void doDbSaveUpdateIncludingStakes(ContinuousTable continuousTable, String stake) throws Exception {
        List<DbSave> dbSaveList = continuousTable.getDbSaveList();
        ...
        if(totalTotalCountLastHand > 0 || totalPre2betCountLastHand > 0 || totalPre3betCountLastHand > 0 || totalPre4bet_up_countLastHand > 0) {
            addOpponentToStakesDbIfNecessary(opponentName, stake);

            Statement st = con.createStatement();

            st.executeUpdate(
                    "UPDATE opponentidentifier_2_0_preflopstats_stakes SET pre2bet = pre2bet + " + totalPre2betCountLastHand +
                            ", pre3bet = pre3bet + " + totalPre3betCountLastHand +
                            ", pre4bet_up = pre4bet_up + " + totalPre4bet_up_countLastHand +
                            ", preTotal = preTotal + " + totalTotalCountLastHand +
                            " WHERE playerName = '" + opponentName + "' AND stakes = '" + stake + "'");
    ...
    private void addOpponentToStakesDbIfNecessary(String opponentName, String stake) throws Exception {
        Statement st = con.createStatement();
        ResultSet rs = st.executeQuery("SELECT * FROM opponentidentifier_2_0_preflopstats_stakes WHERE playerName = '" + opponentName + "' AND stakes = '" + stake + "';");

        if(!rs.next()) {
            Statement st2 = con.createStatement();
            st2.executeUpdate("INSERT INTO opponentidentifier_2_0_preflopstats_stakes (playerName, stakes) VALUES ('" + opponentName + "', '" + stake + "')");
            st2.close();

            System.out.println("Opponent: " + opponentName + " was not present yet in preflopstats_stakes db for stakes: " + stake + " . Added the opponent");
        }
